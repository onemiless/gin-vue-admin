    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column
          align="left"
          label="打样状态"
          prop="proofingCondition"
          width="120"
        />
        <el-table-column
          align="left"
          label="打样特殊说明"
          prop="specialInstructionsForProofing"
          width="120"
        />
        <el-table-column
          align="left"
        ...
        <el-table-column align="left" label="发出日期" width="180">
          <template #default="scope">{{
            formatDate(scope.row.dateIssued)
          }}</template>
        </el-table-column>
        <el-table-column
          align="left"
          label="客户质量判定"
          prop="customerQualityJudgment"
          width="120"
        />
        <el-table-column
          align="left"
          label="客户质量说明"
          prop="customerQualityStatement"
          width="120"
        />
        <el-table-column
          align="left"
        ...
        >
          <template #default="scope">
            <el-button
              ...
              icon="edit"
              class="table-button"
              @click="updateProofingInformationFunc(scope.row)"
              >变更</el-button
            >
            <el-button
              type="primary"
    ...
    >
      ...
      >
        ...
        <el-form-item label="入货单位:" prop="incomingUnit">
          <el-input v-model="formData.incomingUnit" disabled />
        </el-form-item>
        <el-form-item label="打样单号:" prop="numberOfProofing">
          <el-input v-model="formData.numberOfProofing" disabled />
        </el-form-item>
        <el-divider content-position="left">打样信息</el-divider>
        <el-form-item label="接收日期:" prop="reciveDate">
          <el-date-picker
            v-model="formData.reciveDate"
            type="date"
            placeholder="选择日期"
            :clearable="true"
            @change="getSN"
        ...
        <el-form-item label="预计完成日期:" prop="estimatedDateOfCompletion">
          <el-date-picker
            v-model="formData.estimatedDateOfCompletion"
            type="date"
            placeholder="选择日期"
            :clearable="true"
          />
        ...
        <el-form-item label="打样方式:" prop="proofingMethod">
          <el-select
            v-model="formData.proofingMethod"
            placeholder="请选择打样方式"
            :clearable="true"
          >
            <el-option
        ...
        <el-form-item label="打样状态:" prop="proofingCondition">
          <el-select
            v-model="formData.proofingCondition"
            placeholder="请选择打样状态"
            :clearable="true"
          >
            <el-option
        ...
        >
          <el-input
            v-model="formData.specialInstructionsForProofing"
            :clearable="true"
            placeholder="请输入打样特殊说明"
          />
        </el-form-item>
        <el-form-item label="打样图纸:" prop="proofDrawing">
          <el-select
            v-model="formData.proofDrawing"
            placeholder="请选择打样图纸"
            :clearable="true"
          >
            <el-option
        ...
        <el-form-item label="是否留样:" prop="isReservedSample">
          <el-select
            v-model="formData.isReservedSample"
            placeholder="请选择是否留样"
            :clearable="true"
          >
            <el-option
              v-for="item in ['不可留', '不可破坏', '常规留样8PCS']"
              :key="item"
              :label="item"
              :value="item"
            />
        ...
        <el-form-item label="入货量:" prop="goodsReceived">
          <el-input-number
            v-model="formData.goodsReceived"
            :precision="3"
            :clearable="true"
          />
        </el-form-item>
        <el-divider content-position="left">质量相关</el-divider>
        <el-form-item label="佳马质量判定:" prop="qualityJudgment">
          <el-select
            v-model="formData.qualityJudgment"
            placeholder="请选择佳马质量判定"
            :clearable="true"
          >
            <el-option
              v-for="item in ['合格', '不合格', '确认中']"
              :key="item"
              :label="item"
              :value="item"
            />
        ...
        <el-form-item label="佳马盐雾判定:" prop="saltSprayDetermination">
          <el-input
            v-model="formData.saltSprayDetermination"
            :clearable="true"
            placeholder="请输入佳马盐雾判定"
          />
        </el-form-item>
        ...
        <el-form-item label="客户质量判定:" prop="customerQualityJudgment">
          <el-select
            v-model="formData.customerQualityJudgment"
            placeholder="请选择客户质量判定"
            :clearable="true"
          >
            <el-option
              v-for="item in ['合格', '不合格', '确认中']"
        ...
        <el-form-item label="客户质量说明:" prop="customerQualityStatement">
          <el-input
            v-model="formData.customerQualityStatement"
            :clearable="true"
            placeholder="请输入客户质量说明"
          />
        </el-form-item>
        <el-form-item label="打样状态:" prop="ProofingStatus">
          <el-input
            v-model="formData.ProofingStatus"
            :clearable="true"
            placeholder="请输入打样状态"
          />
        </el-form-item>
        <el-form-item label="交期判断:" prop="deliveryJudgment">
          <el-input
            v-model="formData.deliveryJudgment"
            :clearable="true"
            placeholder="请输入交期判断"
          />
        </el-form-item>
...
//获取流水号
const getSN = async () => {
  const date = new Date(formData.value.reciveDate);
  ...
  const mm = m < 10 ? "0" + m : m;
  const dd = d < 10 ? "0" + d : d;
  const paramStr = formData.value.prefix + yy + mm;
  const param = {
    tableName: "proofing_information",
    param: paramStr,
    filedName: "left(number_of_proofing,13)",
  };
  // const res = await getEntryNumber({tableName : "cost_collection",param : param,filedName:"left(number_of_plan,13)"})
  const res = await getEntryNumber(param);
  // console.log(res)
  if (res.code === 0) {
    formData.value.SN = res.data.number;
    //打样单号
    formData.value.numberOfProofing =
      paramStr +
      dd +
      "-" +
      formData.value.SN;
...
const getTableData = async () => {
  const table = await getProofingInformationList({
    page: page.value,
