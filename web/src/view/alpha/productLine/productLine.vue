<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="产线编号" prop="productNumber">
         <el-input v-model="searchInfo.productNumber" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="产线编号" prop="productNumber" width="120" />
        <el-table-column align="left" label="产线名称" prop="productName" width="120" />
        <el-table-column align="left" label="所属分部" prop="subdivision" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.subdivision,BranchOptions) }}
            </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="描述" prop="remark" width="120" />
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateProductLineFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getProductLineList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteProductLineByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateProductLineFunc = async(row) => {
    const res = await findProductLine({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteProductLineFunc = async (row) => {
    const res = await deleteProductLine({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
