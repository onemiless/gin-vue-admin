        ...
        >
          <template #default="scope">
            <el-button
              ...
              icon="edit"
              class="table-button"
              @click="updateCostCollectionFunc(scope.row)"
              >变更</el-button
            >
            <el-button
              type="primary"
    ...
    >
      ...
      >
        ...
        <el-form-item label="策划类型:" prop="typeOfPlan">
          <el-select
            v-model="formData.typeOfPlan"
            placeholder="请选择策划类型"
            disabled
          >
            <el-option
              v-for="item in ['新品策划', '重复策划']"
              :key="item"
              :label="item"
              :value="item"
            />
        ...
        <el-form-item label="策划接收日期:" prop="dateReciveOfPlan">
          <el-date-picker
            v-model="formData.dateReciveOfPlan"
            type="date"
            placeholder="选择日期"
            :clearable="true"
          />
        ...
        <el-form-item label="策划发出日期:" prop="dateIssuedOfPlan">
          <el-date-picker
            v-model="formData.dateIssuedOfPlan"
            type="date"
            placeholder="选择日期"
            :clearable="true"
            @change="dateIssuedOfPlanChange"
          />
        </el-form-item>
        <el-form-item label="策划依据:" prop="planBasis">
          <el-select
            v-model="formData.planBasis"
            placeholder="请选择策划依据"
            :clearable="true"
          >
            <el-option
              v-for="item in ['图纸', '图纸和标准', '标准']"
              :key="item"
              :label="item"
              :value="item"
            />
        ...
        <el-form-item label="主要策划线别:" prop="mainPlanLine">
          <el-select
            v-model="formData.mainPlanLine"
            placeholder="请选择主要策划线别"
            :clearable="true"
          >
            <el-option
              v-for="(item, key) in APPLineOptions"
              :key="key"
              :label="item.label"
              :value="item.value"
            />
...
const calproductionCapacity = () => {
  formData.value.productionCapacity =
    (((12 - 1) * 3600) / formData.value.beat) *
    formData.value.baskets.toFixed(3);
};
//获取唯一追踪号
...
const getSN = async() => {
  ...
  const res = await getEntryNumber({tableName:"cost_collection"})
  // console.log(res)
  if (res.code === 0) {
     formData.value.SN = res.data.number
  }
...
//发出日期填写后获取流水号
const dateIssuedOfPlanChange = async()=>{
  // console.log("param")
  if(formData.value.dateIssuedOfPlan && formData.value.prefix ){
    //格式化dateIssuedOfPlan为yyyyMMdd格式
    const date = new Date(formData.value.dateIssuedOfPlan)
    ...
    const mm = m < 10 ? '0' + m : m
    const dd = d < 10 ? '0' + d : d
    const param = formData.value.prefix  + yy + mm + dd
    // console.log(param)
    ...
    if (res.code === 0) {
      ...
      formData.value.SN = res.data.number
    //策划单号
    formData.value.numberOfPlan = param+"-" +res.data.number;
    }


  }else{
    ElMessage.error('前缀或发出日期为空')
  }
}
...
const getTableData = async () => {
  const table = await getCostCollectionList({
    page: page.value,
