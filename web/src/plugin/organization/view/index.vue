  ...
  <div class="organization">
    ...
    <div class="gva-organization-box">
      <div class="gva-organization-box-left">
        <div class="toolbar">
          <el-button type="primary" @click="addOrg(0)">同步企业微信部门</el-button>
        </div>
        <div class="tree-body">
          <el-tree ref="treeRef" :data="treeData" show-checkbox node-key="ID" :props="defaultProps" lazy
            :load="loadDeptData" highlight-current @current-change="getNowOrg">
            <template #default="{ node, data }">
              <span class="tree-body-tree-node">
                <el-tooltip class="box-item" effect="dark" show-after="600" :content="node.label" placement="top-start">
          ...
          <el-table :data="userTable" @selection-change="handleSelectionChange">
            ...
            <el-table-column label="操作列" min-width="220">
              <template #default="{ row }">
                ...
                <el-button v-if="!row.isAdmin" link type="primary" @click="setAdmin(row.sysUser.ID, true)">
                  设置管理员</el-button>
    ...
    <el-dialog v-model="orgDialog" title="组织管理">
      <el-form :model="orgForm" label-width="120px">
        <el-form-item label="父组织">
          <el-select v-model="orgForm.parentID" filterable remote reserve-keyword placeholder="请选择组织"
            :remote-method="remoteMethod" :loading="loading">
            <el-option label="根组织" :value="0" />
    ...
    <el-dialog v-model="orgUserDialog" title="组织人员">
      <el-form v-model="orgUserForm" label-width="120px">
        <el-form-item label="组织">
          <el-select v-model="orgUserForm.organizationID" disabled placeholder="请选择目标组织">
            <el-option v-for="item in data" :key="item.ID" :label="item.name" :value="item.ID" />
          </el-select>
        </el-form-item>
        <el-form-item label="成员">
          <el-select v-model="orgUserForm.sysUserIDS" placeholder="请选择需要加入组织的成员" multiple filterable>
            <el-option v-for="item in userList" :key="item.ID" :disabled="disabledUserMap[item.ID]" :label="item.nickName"
              :value="item.ID" />
          </el-select>
        </el-form-item>
      </el-form>
    ...
    <el-dialog v-model="transferOrgUserFlag" title="更换组织">
      <el-form v-model="targetOrg" label-width="120px">
        <el-form-item label="父组织">
          <el-select v-model="targetOrg.toOrganizationID" filterable remote reserve-keyword placeholder="请选择组织"
            :remote-method="remoteMethod" :loading="loading">
            <el-option label="根组织" :value="0" />
            <el-option v-for="item in data" :key="item.ID" :label="item.name" :value="item.ID" />
          </el-select>
